...
public class ParcelaDAO {
    ...
    public static int insertar(int idTerreno, String ubicacion, int limiteBase, int limiteAltura, Date fechaRegistro) {
        try {
            conn = ConexionDB.getConn();
            PreparedStatement pS = conn.prepareStatement(
                    "INSERT INTO parcelas (idTerreno, ubicacion, limiteBase, limiteAltura, fechaRegistro) VALUES (?,?,?,?,?)");
    ...
    public static int eliminar(int idParcela) {
        try {
            conn = ConexionDB.getConn();
    ...
    public static int actualizar(int id, int idTerreno, String ubicacion, int limiteBase, int limiteAltura,
            Date fechaRegistro) {
        try {
            conn = ConexionDB.getConn();
            PreparedStatement pS = conn.prepareStatement(
                    "UPDATE parcelas SET ubicacion=?, limiteBase=?, limiteAltura=?, fechaRegistro=? WHERE id=?");
    ...
    public static Parcela buscarPorId(int idParcela) {
        ...
        try {
            ...
            if (rs.next()) {
                parcela = new Parcela(rs.getInt(1), rs.getInt(2), rs.getString(3), rs.getInt(4), rs.getInt(5),
                        rs.getDate(6));
            }
            pS.close();
            conn.close();
    ...
    public static ArrayList<Parcela> listarTodos() {
        ...
        try {
            ...
            ResultSet rs = pS.executeQuery();
            while (rs.next()) {
                Parcela parcela = new Parcela(rs.getInt(1), rs.getInt(2), rs.getString(3), rs.getInt(4), rs.getInt(5),
                        rs.getDate(6));
